...
def index(request):
    ...
    sharmistha           = {'allProds': allProds}
    return render(request, 'shop/index.html', sharmistha)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    sharmistha = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        sharmistha = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', sharmistha)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            sharmistha_dict['CHECKSUMHASH'] = Checksum.generate_checksum(sharmistha_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'sharmistha_dict': sharmistha_dict})
